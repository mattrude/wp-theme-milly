...
add_action( 'admin_init', 'milly_theme_options_init' );
add_action( 'admin_menu', 'milly_theme_options_add_page' );
 ...
 */
function milly_theme_options_init(){
	register_setting( 'milly_options', 'milly_theme_options', 'milly_theme_options_validate' );
}

/**
 * Load up the menu page
 */
function milly_theme_options_add_page() {
	add_theme_page( __( 'Theme Options' ), __( 'Theme Options' ), 'edit_theme_options', 'theme_options', 'milly_theme_options_do_page' );
}

/**
 * Return array for our color schemes
 */
function milly_color_schemes() {
	$color_schemes = array(
		'light' => array(
			'value' =>	'light',
			'label' => __( 'Light' )
...
add_action( 'init', 'milly_default_options' );

/**
 * Create the options page
 */
function milly_theme_options_do_page() {
	...
	?>
	<div class="wrap">
		<?php screen_icon(); echo "<h2>" . sprintf( __( '%1$s Theme Options', 'milly' ), get_current_theme() )
		 . "</h2>"; ?>
		...
		<form method="post" action="options.php">
			<?php settings_fields( 'milly_options' ); ?>
			<?php $options = get_option( 'milly_theme_options' ); ?>
			...
			<table class="form-table">
				...
				<tr valign="top"><th scope="row"><?php _e( 'Color Scheme' ); ?></th>
					<td>
						<select name="milly_theme_options[color_scheme]">
							<?php
								$selected = $options['color_scheme'];
								$p = '';
								$r = '';

								foreach ( milly_color_schemes() as $option ) {
									$label = $option['label'];
						...
						<label class="description" for="milly_theme_options[color_scheme]"><?php _e( 'Select a default color scheme' ); ?></label>
					</td>
				</tr>
				 ...
				 */
				?>
				<tr valign="top" id="milly-layouts"><th scope="row"><?php _e( 'Default Layout' ); ?></th>
					<td>
						<fieldset><legend class="screen-reader-text"><span><?php _e( 'Default Layout' ); ?></span></legend>
						<?php
							if ( ! isset( $checked ) )
								$checked = '';
							foreach ( milly_layouts() as $option ) {
				...
				<?php
				/**
				 * Milly Aside Category
				 */
				?>
				<tr valign="top"><th scope="row"><?php _e( 'Aside Category' ); ?></th>
					<td>
						<select name="milly_theme_options[aside_category]">
						...
						<label class="description" for="milly_theme_options[aside_category]"><?php _e( 'Select a category to use for shorter aside posts' ); ?></label>
					</td>
				</tr>

				<?php
				/**
				 * Milly Gallery Category
				 */
				?>
				<tr valign="top"><th scope="row"><?php _e( 'Gallery Category' ); ?></th>
					<td>
						<select name="milly_theme_options[gallery_category]">
							<option value="0"><?php _e( 'Select a category &hellip;' ); ?></option>
							<?php
						...
						<label class="description" for="milly_theme_options[gallery_category]"><?php _e( 'Select a category to use for posts with image galleries' ); ?></label>
					</td>
				</tr>
